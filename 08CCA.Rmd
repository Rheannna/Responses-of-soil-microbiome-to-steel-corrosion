...
```{r}
   ...
   output1
   str(output1)
   d=output1$sites;  ##样本坐标
   d <- as.data.frame(d)
   e=output1$cont$importance; ##特征值，解释度 #eigenvals(C.whole)
    e <- as.data.frame(e)
   f=output1$biplot;  ##环境因子坐标
   f <- as.data.frame(f)
```
...
library(hier.part)
library(rdaenvpart)
#pieplot="tv"表明占能被解释的比例,tev表示占总解释的比例
...
```{r}
#加标签 + geom_text_repel(data = samplenew, aes(CCA1, CCA2, label=group_Cu2), size=1.5, box.padding=unit(0.2, "lines"), point.padding=unit(0.2, "lines"), segment.color = "grey50", segment.size = 0.2, arrow = arrow(length=unit(0.01, "npc")),force = 1, max.iter = 3e3)
